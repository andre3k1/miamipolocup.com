	...
	public function responsive_video ($attributes, $content = null)
	{
        ...
        if ($result) {
            if ($result['provider'] == 'youtube') {
                $output = '<div class="w-video"><iframe width="420" height="315" src="//www.youtube.com/embed/' . $result['id'] . '" frameborder="0" allowfullscreen></iframe></div>';
            } elseif ($result['provider'] == 'vimeo') {
                $output = '<div class="w-video"><iframe src="//player.vimeo.com/video/' . $result['id'] . '?byline=0&amp;color=cc2200" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
            }
        } else {
            global $wp_embed;
            $embed = $wp_embed->run_shortcode('[embed]'.$attributes['link'].'[/embed]');

            $output = '<div class="w-video">' . $embed . '</div>';
        }
...
//		$output .= "<script>
//		jQuery(document).ready(function(){if (jQuery('#gallery_".$rand_id."').length) console.log(1);
