			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' .  'Return to Required Plugins Installer' . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' .  'Return to the Dashboard' . '">' .  'Return to the Dashboard' . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf(  'The following plugin was activated successfully: %s.', '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' .  'Dismiss this notice' . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] =  'External Link';
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] =  'Private Repository';
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] =  'Pre-Packaged';
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] =  'WordPress Repository';
				}

				$table_data[$i]['type'] = $plugin['required'] ?  'Required' :  'Recommended';

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s',  'Not Installed' );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s',  'Installed But Not Activated' );
		...
		public function no_items() {
			...
			printf(  'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  =>  'Install',
				'tgmpa-bulk-activate' =>  'Activate',
			);
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          =  'Install package not available.';
				$this->strings['downloading_package'] =  'Downloading install package from <span class="code">%s</span>&#8230;';
				$this->strings['unpack_package']      =  'Unpacking the package&#8230;';
				$this->strings['installing_package']  =  'Installing the plugin&#8230;';
				$this->strings['process_failed']      =  'Plugin install failed.';
				$this->strings['process_success']     =  'Plugin installed successfully.';
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  =  'Plugin activation failed.';
				$this->strings['activation_success'] =  'Plugin activated successfully.';
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					...
					$this->upgrader->strings['skin_update_successful']    =  '%1$s installed and activated successfully.' . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' .  'Show Details' . '</span><span class="hidden">' .  'Hide Details' . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          =  'All installations and activations have been completed.';
					$this->upgrader->strings['skin_before_update_header'] =  'Installing and Activating Plugin %1$s (%2$d/%3$d)';
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        =  'The installation process is starting. This process may take a while on some hosts, so please be patient.';
					...
					$this->upgrader->strings['skin_update_successful']    =  '%1$s installed successfully.' . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' .  'Show Details' . '</span><span class="hidden">' .  'Hide Details' . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          =  'All installations have been completed.';
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' .  'Return to the Dashboard' . '">' .  'Return to the Dashboard' . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
